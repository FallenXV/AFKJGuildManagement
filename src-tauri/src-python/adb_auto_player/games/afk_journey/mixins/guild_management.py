...
class MemberListConfig:
    ...
    gender_templates: (
        dict[str, dict[str, npt.NDArray[np.uint8] | int | None]] | None
    ) = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.gender_templates is None:
            templates: dict[str, dict[str, npt.NDArray[np.uint8] | int | None]] = {}
            for label, pad in {"male": 8, "female": 10}.items():
                try:
                    raw_img = IO.load_image(Path(f"guild/{label}.png"), grayscale=True)
...
def compute_badge_shift(
    name_gray: np.ndarray,
    templates: dict[str, dict[str, npt.NDArray[np.uint8] | int | None]] | None,
    thr: float,
) -> int:
    if not templates:
        return 0
    name_gray_u8: npt.NDArray[np.uint8] = np.asarray(name_gray, dtype=np.uint8)
    fixed_shift_map = {"male": 36, "female": 48}
    best_label, best_val = None, 0.0

    for label, item in templates.items():
        tpl_raw = item.get("img")
        if tpl_raw is None:
            continue
        tpl: npt.NDArray[np.uint8] = np.asarray(tpl_raw, dtype=np.uint8)
        res = cv2.matchTemplate(name_gray_u8, tpl, cv2.TM_CCOEFF_NORMED)
        if res.size == 0:
            continue
